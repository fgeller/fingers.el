...
** Recent changes
   ...
   - Added + and - bindings to increment and decrement number under point.

   - Switched bindings for region specifiers "line" and "till line end"

   - Search prefixes and bindings for repition changed: =uu= / =pp= to start
     searching, =U= / =P= to repeat search.

   - Added bindings for =universal-argument=, and commands to pop local and global
     marks.

   - Added hook for customizing key-bindings. Simplifies =eval-buffer= to test new
     bindings while maintaining custom bindings.
